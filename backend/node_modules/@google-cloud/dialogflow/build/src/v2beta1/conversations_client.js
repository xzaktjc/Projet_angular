...
class ConversationsClient {
    ...
    initialize() {
        ...
        const conversationsStubMethods = ['createConversation', 'listConversations', 'getConversation', 'completeConversation', 'ingestContextReferences', 'batchCreateMessages', 'listMessages', 'suggestConversationSummary', 'generateStatelessSummary', 'generateStatelessSuggestion', 'searchKnowledge', 'generateSuggestions'];
    ...
    batchCreateMessages(request, optionsOrCallback, callback) {
        ...
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('batchCreateMessages response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.batchCreateMessages(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('batchCreateMessages response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
